...
- The Map view can switch between the existing 2D projection and a CesiumJS WGS-84 3D globe. The globe renders satellites at true altitude, projected ground paths, the Central Alaska station, and live access lines while sharing simulation time and satellite selection with the 2D map. Dedicated − / + controls provide reliable zoom when browser or trackpad pinch gestures are unavailable.
